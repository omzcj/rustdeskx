...
SOURCE_BUILD_VERSION="$(/usr/bin/plutil -extract CFBundleVersion raw "$SOURCE_PLIST")"
[[ "$SOURCE_VERSION" == "$UPSTREAM_VERSION" ]] || die "expected RustDesk $UPSTREAM_VERSION, got $SOURCE_VERSION"
[[ "$SOURCE_BUILD_VERSION" == "$UPSTREAM_BUILD_VERSION" ]] || die "expected RustDesk build $UPSTREAM_BUILD_VERSION, got $SOURCE_BUILD_VERSION"
OUTPUT_BUILD_VERSION="${UPSTREAM_BUILD_VERSION}.${RELEASE_REVISION}"
...
/usr/bin/plutil -replace CFBundleVersion -string "$OUTPUT_BUILD_VERSION" "$OUTPUT_PLIST"
...
ACTUAL_VERSION="$(/usr/bin/plutil -extract CFBundleShortVersionString raw "$OUTPUT_PLIST")"
ACTUAL_BUILD_VERSION="$(/usr/bin/plutil -extract CFBundleVersion raw "$OUTPUT_PLIST")"
...
[[ "$ACTUAL_VERSION" == "$UPSTREAM_VERSION" ]] || die "app version verification failed"
[[ "$ACTUAL_BUILD_VERSION" == "$OUTPUT_BUILD_VERSION" ]] || die "build version verification failed"
[[ "$(/usr/bin/lipo -archs "$OUTPUT_APP/Contents/MacOS/$EXECUTABLE_NAME")" == "arm64" ]] || die "main executable is not arm64-only"

info "Created $OUTPUT_APP version $ACTUAL_VERSION ($ACTUAL_BUILD_VERSION)"
